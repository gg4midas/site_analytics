...
# 站点流量统计（埋点版）启动脚本
# 用法：
#   bash start.sh                       # 默认端口 8899（账号登录鉴权，首个注册用户自动成为管理员）
#   PORT=8899 bash start.sh
#   bash start.sh --port 8899
...
# 透传命令行参数（--port 等；--create-admin 直接透传给 app.py）
while [ $# -gt 0 ]; do
  case "$1" in
    --port)  PORT="$2"; shift 2 ;;
    --token) echo "[提示] --token 已废弃（v1.5.0 起账号登录鉴权），参数已忽略。" >&2; shift 2 ;;
