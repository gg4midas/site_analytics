...
### v1.3.9（2026-08-13）
- **部署令牌（防 tracker 盗用 / 抗压）**：`/api/event` 端点新增可选的部署令牌校验。服务端设 `SA_DEPLOY_KEY`（或 `--deploy-key`）后，tracker 须通过 `data-key` 随每次上报带回正确令牌，否则请求在校验阶段即被 `403` 拒绝，**不触发任何 GeoIP/ASN 解析与落库**，从根本上消除「脚本被盗用后在高流量陌生站点产生服务器压力」的问题。
  - 默认「宽松模式」：已知站点（已在 `sites` 表或已有历史事件的站点）照常接收，**现有监控零中断**；仅拦截陌生（新）站点，保证数据平滑对接。
  - 「严格模式」（`--require-key` 或 `SA_REQUIRE_KEY=1`，全站重新嵌入带令牌脚本后开启）：所有站点都必须携带正确令牌，连已知站点被盗用上报也拒绝，彻底锁死。
  - 说明：令牌写在公开 JS 中，仅能挡「直接打 endpoint / 陌生域名盗用」这类最常见滥用，并可随时轮换/吊销；对「完整盗走含令牌的 tracker.js 并原样复用」的针对性攻击无能为力（客户端无法保密），那种情况需配合反向代理层按来源 IP/域名白名单等额外手段。详见下方「部署令牌」章节。
...
`/api/event` 是公开端点（埋点跑在访客浏览器里，无法强制登录）。但公开的 `tracker.js` 一旦被他人整段复制、嵌到自己的高流量站点，你的服务器就要为每个伪造访客做 GeoIP/ASN 解析 + 落库，产生不必要的压力。**部署令牌**让服务端只接受「携带正确令牌」的上报，把压力挡在落库之前。

### 工作原理

- 服务端通过环境变量（或启动参数）持有令牌 `SA_DEPLOY_KEY`。
- 埋点脚本通过 `data-key="你的令牌"` 注入令牌，并随每次上报带回。
- 服务端在落库前校验：非法/缺失令牌的**陌生站点**请求直接 `403` 拒绝，不产生任何 GeoIP/落库开销。

### 两种模式

| 模式 | 如何开启 | 已知站点（已有数据） | 陌生（新）站点 |
|---|---|---|---|
| **宽松模式（默认）** | 仅设 `SA_DEPLOY_KEY` | 照常接收（无需令牌） | 必须带正确令牌，否则拒绝 |
| **严格模式** | 再加 `--require-key` / `SA_REQUIRE_KEY=1` | 必须带正确令牌，否则拒绝 | 必须带正确令牌，否则拒绝 |

> 宽松模式的设计目的就是**平稳迁移**：你现有的站点（bio-starch.com 等）已经在 `sites` 表 / 事件库里，属于「已知站点」，即使还没注入 `data-key` 也照常上报、数据不断。只有**陌生域名**会被拦截。因此开启令牌后现有监控零中断。

### 启用步骤（推荐路径）

1. **生成令牌**：在服务器上执行 `bash sa-console.sh key`，会输出一个随机令牌及完整配置示例。
2. **服务端启用（宽松模式）**：在 systemd 单元的环境变量里加 `SA_DEPLOY_KEY=你的令牌`（或直接 `--deploy-key 你的令牌` 启动）。重启服务。
   - 此刻：你的已知站点照常工作；任何陌生站点盗用脚本都会被拒绝。
3. **全站重新嵌入**：把 `data-key="你的令牌"` 加进每个站点的埋点脚本：
   ```html
   <script src="https://你的分析域名/tracker.js" data-site="example.com" data-key="你的令牌" defer></script>
   ```
4. **（可选）彻底锁死**：确认所有自有站点都已带 `data-key` 正常上报后，启动加 `--require-key`（或环境变量 `SA_REQUIRE_KEY=1`）。此后连已知站点也必须带正确令牌，被盗用上报也会被拒。
5. **轮换 / 吊销**：直接换一个 `SA_DEPLOY_KEY` 值并重新嵌入即可；旧令牌立即失效。
...
- 不启用 `SA_DEPLOY_KEY` 时，端点行为等同于旧版（完全开放），向后兼容。
