...
- **开源协议版权更新 + 多项 bug 修复**：
  ...
  - **修复趋势图在 UTC / 非东八区服务器上整体为空**：统计分桶改用配置时区偏移（`'%+d seconds'`），与 Python 侧 `TZ_BEIJING` 保持一致，不再依赖取操作系统时区的 SQLite `'localtime'`。
  - **修复「过去 2 天」范围被误标为「2 天之前」**：后端范围标签、前端下拉、英文文案三处统一为「过去 2 天 / Last 2 days」。
  - **修复屏蔽访客列表越权返回**：传入主域但无对应子域时不再误返回全部屏蔽记录（改为返回空列表）。
  - **修正潜在询盘访客的误导性注释**（实为 OR 语义：任一来源关键词命中即视为潜在询盘）。

### v1.4.6（2026-08-14）
- **访客标签新增来源筛选**：访客列表表头新增「来源筛选」下拉，可直接按「来源类型」（搜索引擎 / AI 工具 / 社交媒体 / 外部链接 / 直接访问）或「来源域名」筛选访客，无需再跳转到来源页点击锚文本；选项随当前站点数据自动生成，与来源页锚点、筛选标签、清除按钮完全联动。中英文界面同步。

### v1.4.5（2026-08-14）
- **移除 CSV 导出、清理版本说明**：移除仪表盘顶栏「导出 CSV」按钮（评估后无实际使用场景）；同步去掉版本说明中的内部优化项编号，改以平实功能描述呈现。

### v1.4.4（2026-08-14）
- **性能与健壮性优化（单文件零依赖、数据库零迁移）**：
  - **趋势聚合去 N+1**：`get_stats` 的逐小时 / 逐天循环改为单条 `GROUP BY strftime(...)` 聚合，24 小时由 24 次查询降为 1 次、7 天小时级由 168 次降为 1 次，Dashboard 刷新负载降一个量级。
  - **统计结果 TTL 缓存**：进程内 45 秒缓存（key = 站点 + 区间），自动刷新 / 多标签页不再每次重算 30+ 条聚合 SQL；屏蔽访客（block/unblock/机房回填）后立即失效缓存，数据口径实时准确。
  - **连接防泄漏**：`get_stats` 改用 `_conn_cursor()` 上下文管理器（`with`），异常路径也确保连接关闭，杜绝并发下的连接句柄泄漏。
  - **复合索引**：新增 `idx_events_site_type_ts(site, type, ts)`，覆盖主导查询 `WHERE site IN(...) AND type='pageview' AND ts>=? AND ts<?`，使范围走完索引。
  - **可见事件视图反连接**：`visible_events` 由 `NOT IN (SELECT ...)` 子查询改为 `LEFT JOIN ... WHERE bv.visitor IS NULL` 反连接，屏蔽表变大时统计不再每条重跑子查询。
  - **站点列表缓存**：`get_sites` 加 30 秒 TTL 缓存，新增 / 删除站点时失效。
  - **前端体验**：访客表分块渲染（每批 80 行 +「加载更多」），避免长列表一次性渲染卡顿；顶栏「导出 CSV」一键导出趋势 / 来源 / 页面 / 设备 / 地域 / 访客为 Excel 可打开的 UTF-8 CSV。

### v1.4.3（2026-08-14）
- **后端可靠性加固（生产必修，零依赖 / 数据库兼容）**：
  - **SQLite 并发写入**：`_conn()` 默认开启 `WAL` 日志模式 + `busy_timeout=5000` + `synchronous=NORMAL`，消除高并发下 `database is locked` 导致 tracker 上报**静默丢事件**的问题；旧 `events.db` 直接复用，零迁移风险。
  - **请求体上限**：所有 `POST` 接口统一经 `_read_body()` 安全读取，请求体上限 **64KB**，并对非整数 `Content-Length` 容错（不再返回 500），防慢速 / 超大体请求占满线程池（slowloris / DoS）。
  - **超时与守护线程**：`Handler.timeout=15` 断掉慢客户端，`ThreadingHTTPServer(daemon_threads=True)` 避免关闭期挂起。
  - **时序侧信道**：`_deploy_ok` 令牌比较由 `==` 改为 `hmac.compare_digest`，消除令牌校验的时序侧信道。

### v1.4.2（2026-08-14）
- **UI 设计评审系统性修复（14 项）**：
  - **功能性 bug（3）**：修复 `--fg` 缺失导致深色模式 textarea 黑字黑底不可读；修复 `--track` 缺失导致进度条轨道在深色下为刺眼浅块；修复 `.code-box` 硬编码黑底导致浅色主题弹窗出现沉重黑块（均改为由设计 Token 控制）。
  - **一致性（5）**：散落硬编码色（进度条渐变、站点标签紫、嫌疑徽标橙、性能条/分段计数白底）全部抽为 `--accent/--lead/--suspect/--track` 等 Token，深浅主题自动协调；顶栏之外的控件（刷新按钮、实时窗口下拉、自定义日期输入）统一为 36px 高度；主部署码框改为与每站点一致的两列三按钮；图表高度收口为 `--chart-sm/md/lg` 三档；新增间距/字号/圆角/控件高度/过渡标尺 Token。
  - **体验与无障碍（6）**：新增全局 `:focus-visible` 焦点环；提亮 `--faint` 使小字达 WCAG AA 4.5:1；模态增加 `role="dialog"`/`aria-modal`/`aria-label`、ESC 关闭与 Tab 焦点陷阱；合并性能页重复说明为单处；导航在窄屏改为独立横向滚动且刷新按钮固定；新增顶部加载进度条与内容淡入加载态。
